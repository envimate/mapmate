...
public final class ListCollectionDefinitionFactory implements DefinitionFactory {

    public static DefinitionFactory listFactory() {
        return new ListCollectionDefinitionFactory();
    }

    @Override
    public Optional<Definition> analyze(final FullType type) {
        if (!List.class.isAssignableFrom(type.type())) {
            return empty();
        }
        ...
        final CollectionDeserializer deserializer = listDeserializer();
