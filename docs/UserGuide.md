...
with a lot of prerequisites. The ones it comes with, are explained in this chapter   
...
Also, include this flag in your IDE's javac configuration, and make sure to rebuild your project afterward the 
...
Serialized Objects by generating private constructors, `equals` and `hashCode` and `toString`. Checkout their web site 
...
```java
MyObject myObject = mapMate.deserializeYaml(myObjectAsXml, MyObject.class);
``` 
...
```java
MyObject myObject = mapMate.deserializeYaml(myObjectAsSomething, MyObject.class, MarshallingType.marshallingType("YOUR_CUSTOM_FORMAT"));
``` 
...
for a hero that will be brave enough to fill the black hole in it's center with documentation. Mysterious and powerful 
features like [Injection Support](../core/src/main/java/com/envimate/mapmate/injector/InjectorLambda.java) or
`Function<Map<String, Object>, Map<String, Object>> serializedPropertyInjector` are rumored to be found there, but 
...
There are cases, where class path scanning is not a desired feature, e.g. high security environments or serverless
...
With package scanning disabled, you need to either register your types manually or provide a list of classes, that 
MapMate will detect your Custom Primitives and Serialized Objects from. 
See 
...
A serialization methods is public, returns an instance of String and takes no arguments. A deserialization method is 
...
## Recipes
In the real world, a good recipe provides instructions and ingredients on how to cook stuff, pan cakes for instance.
If you've had some friends over for a pan cake party, and you've made good pan cakes, chances are, that you are asked
for a recipe. What happens if you share it, is essentially making a copy of the recipe. The same approach is taken by 
some IT books, most of them can be identified by `book.name.contains("cookbook")`. The problem with that approach
...
In order to avoid that problem for MapMate, we ship with the `Recipe` interface, which allows everyone, to craft tasty
MapMate recipes and share them as code. This way, you can change the recipe and roll out the change using your favorite
...
numeric data types or the configuration of Jackson and it's registration as Marshaller in MapMate. They are also a great
...
a few lines of code. It will configure Jackson to not attempt to parse numbers or booleans and instead just use the 
string value as stated in [Concepts](Concepts.md) under `String representation`. Also, it will not serialize properties
...
Although we put a lot of effort into stating that we discourage the use of primitives shipped with the language, we 
understand that sometimes things are different and for these times, MapMate ships with a recipe that will make it map
...
Scanning the class path and analysing which classes are 
Custom Primitives, which are Serialized Objects and which are to ignore is a great way to trade development effort with
CPU effort. But if you intend to run on a serverless platform like AWS Lambda, chances are high, that your project 
only contains a handful of Custom Primitives/Serialized Objects and you want a very fast, optimized application startup.
...
Teaching MapMate how to work with unconventional types is bit more complex and requires a basic understanding
of how MapMate is interacting with your types.

MapMate is behaving like you'd behave when interacting with Custom Primitive, it's calling one of it's methods to obtain
...
converts the string into a type. Given that information, manually registering an unconventional Custom Primitive becomes
straight forward:
```java
MapMate.aMapMate()
                .usingRecipe(manuallyRegisteredTypes()
...
let's dive into the lions Den.

The "easy" way to help MapMate understand how to deal with an unconventional Serialized Object is to provide the type,
a list fields that are supposed to be serialized and a string representing the factory methods name:
```java
MapMate.aMapMate()
                .usingRecipe(manuallyRegisteredTypes()
...
In order to create an instance of Email, MapMate needs the `elements`:
```json
{
...
```
Once MapMate obtained all of the elements, it will call `deserialize` with the target type and a Map similar to 
...
interface. At this point in time, it carries too many internals, is hard to understand and even harder to explain. If
...
In order to create a recipe, one has to understand the 
...
### The Builder Process
One of the goals of MapMate is to be ultra customizable. Another one is ultra short and simple conventions to reduce 
configuration effort. In order to satisfy both of the goals, MapMate is built using a builder that implements the 
following process:
...
3. Ask the `PackageScanner` to list all detection candidates aka the classes, that might be a Custom Primitive, 
...
Checkout the code of [MapMateBuilder](../core/src/main/java/com/envimate/mapmate/builder/MapMateBuilder.java) for more
...
The `cook` method is quite simple, it's basically a call back with the MapMateBuilder instance that can be used to wrap
multiple builder calls into a single Recipe. The
[JacksonMarshaller](../core/src/main/java/com/envimate/mapmate/builder/recipes/marshallers/jackson/JacksonMarshaller.java)
is a great example for Recipes implementing only that method.
